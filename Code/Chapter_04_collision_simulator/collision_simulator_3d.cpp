...
#include "Base/GasConstants.h"

#include "CollisionSimulator3D/ContainerFactory3D.h"
#include "CollisionSimulator3D/CollisionSimulator3D.h"

#include "MolecularDynamics/IdealGasCalculator.h"
#endif
...
void Collision_simulator_3D_N_random_balls()
{
	...
	Serializer::SaveParticleSimulation3D(MML_PATH_ResultFiles + "collision_sim_3d_example1.txt", box._balls.size(), box._width, box._height, box._depth,
...
void Collision_simulator_3D_two_types_initially_separated()
{
	...
	Serializer::SaveParticleSimulation3D(MML_PATH_ResultFiles + "collision_sim_3d_example2.txt", box._balls.size(), box._width, box._height, box._depth,
		simResult.BallPosList, box.getBallColors(), box.getBallRadii(), dT);
...
void Collision_simulator_3D_ideal_gas_in_100_nm_cube()
{
	...
	Serializer::SaveParticleSimulation3D(MML_PATH_ResultFiles + "ideal_gas.txt", box._balls.size(), box._width, box._height, box._depth,
		simResult.BallPosList, box.getBallColors(), box.getBallRadii(), dT);
...
void Collision_simulator_3D_simulating_air_as_ideal_gas_in_100_nm_cube()
{
	...
	const GasData& O2 = GasConstants::gasByName("O2");
	const GasData& N2 = GasConstants::gasByName("N2");
	const GasData& Ar = GasConstants::gasByName("Ar");
	const GasData& CO2 = GasConstants::gasByName("CO2");
	...
	double radius_N2_nm = 0.3;						// radius of N2 molecule in nm
	...
	double volume_m3 = boxSide * boxSide * boxSide;
	double n_molecules = P * volume_m3 / (k_B * T); // number of molecules using ideal gas law
	...
	Serializer::SaveParticleSimulation3D(MML_PATH_ResultFiles + "air_as_ideal_gas.txt", box._balls.size(), box._width, box._height, box._depth,
		simResult.BallPosList, box.getBallColors(), box.getBallRadii(), dT);
