	...
	public:
		...
		void HandleCollision(int m, int n, double dt)
		{
			if (_collisionRecorder != nullptr)
				_collisionRecorder->RecordCollisionEvent(m, n, dt);
		...
		void SimulateOneStepFast(double dt)
		{
			...
			//std::cout << "Number of balls in each subcontainer:" << std::endl;
			//for (int i = 0; i < M.RowNum(); i++)
			//{
			//	for (int j = 0; j < M.ColNum(); j++)
			//	{
			//		std::cout << "M (" << i << ", " << j << "): " << M(i, j).size() << " ";
			//		// print all indices of balls in that subcontainer
			//		if (!M(i, j).empty())
			//		{
			//			std::cout << " - Indices: ";
			...
			// even thought ... this is LINEAR operation, so it is not a big deal
			for (int i = 0; i < NumBalls; i++)
				_box.CheckAndHandleOutOfBounds(i);
		...
		void SimulateOneStepFastMultithreadThreadPool(double dt)
		{
			_box.SetNumBallsInSubdividedContainer(M.RowNum(), M.ColNum(), M);
		...
		SimResultsCollSim2D Simulate(int numSteps, double timeStep, CollisionSimulatorRunType runType = CollisionSimulatorRunType::RunTypeFastMultithread)
		{
			...
			for (int i = 0; i < numSteps; i++)
			{
				...
				case MPL::RunTypeFastMultithreadTP:
					SimulateOneStepFastMultithreadThreadPool(dt);
					break;
				default:
					break;
				}
		...
		bool Serialize(std::string fileName, const SimResultsCollSim2D& simResults, double dT, int saveEveryNSteps = 1)
		{
			...
			return Serializer::SaveParticleSimulation2D(fileName, numBalls, _box._width, _box._height, simResults.BallPosList, ballColors, ballRadius, dT, saveEveryNSteps);
		}
	};
}
#endif